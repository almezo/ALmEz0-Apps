...
// =========================================================
// ALMEZO REMOTE UI STUDIO CONTROLLER (استوديو تخصيص الواجهة والمشغل)
// =========================================================
const UI_STUDIO_PRESETS = {
    emerald: {
        name: 'أخضر الميزو الملكي',
        primaryColor: '#2e7d32',
        accentColor: '#4caf50',
    ...
    gold: {
        name: 'ذهبي بريميوم فاخر',
        primaryColor: '#b45309',
        accentColor: '#f59e0b',
        glowColor: 'rgba(245, 158, 11, 0.35)',
        bgDark: '#0f0d09',
        bgCard: 'rgba(28, 23, 14, 0.85)',
        borderColor: 'rgba(245, 158, 11, 0.25)',
        textPrimary: '#ffffff'
    },
    violet: {
        name: 'أرجواني سايبربانك',
        primaryColor: '#7c3aed',
        accentColor: '#a855f7',
        glowColor: 'rgba(168, 85, 247, 0.35)',
        bgDark: '#0d0814',
        bgCard: 'rgba(24, 16, 36, 0.85)',
        borderColor: 'rgba(168, 85, 247, 0.25)',
        textPrimary: '#ffffff'
    },
    crimson: {
        name: 'أحمر سينمائي ناري',
        primaryColor: '#b91c1c',
        accentColor: '#ef4444',
        glowColor: 'rgba(239, 68, 68, 0.35)',
        bgDark: '#0d0607',
        bgCard: 'rgba(28, 14, 16, 0.85)',
        borderColor: 'rgba(239, 68, 68, 0.25)',
        textPrimary: '#ffffff'
    },
    titanium: {
        name: 'تيتانيوم وفضي هادئ',
        primaryColor: '#475569',
        accentColor: '#94a3b8',
        glowColor: 'rgba(148, 163, 184, 0.3)',
        bgDark: '#090a0f',
        bgCard: 'rgba(20, 22, 28, 0.85)',
...
window.openUiStudioModal = async function () {
    const modal = document.getElementById('uiStudioModal');
    if (!modal) return;
    modal.style.display = 'flex';

    // 1. Fetch current settings from Firestore or Local Cache
    let currentConfig = null;
    try {
        if (typeof db !== 'undefined') {
            const doc = await db.collection('siteConfig').doc('ui_customization').get();
            if (doc.exists) currentConfig = doc.data();
        }
    } catch (e) {
        console.warn('Failed to fetch remote UI config from Firestore:', e);
    }

    if (!currentConfig && window.AlMeZ0App && typeof window.AlMeZ0App.getRemoteUiConfig === 'function') {
        currentConfig = window.AlMeZ0App.getRemoteUiConfig();
    }
    if (!currentConfig) {
        currentConfig = (window.AlMeZ0App && window.AlMeZ0App.DEFAULT_UI_CONFIG) || UI_STUDIO_PRESETS.emerald;
    }

    // 2. Populate Colors
    setStudioFieldValue('uiPrimaryColor', currentConfig.primaryColor || '#2e7d32');
    setStudioFieldValue('uiPrimaryHex', currentConfig.primaryColor || '#2e7d32');
    setStudioFieldValue('uiAccentColor', currentConfig.accentColor || '#4caf50');
    setStudioFieldValue('uiAccentHex', currentConfig.accentColor || '#4caf50');
    setStudioFieldValue('uiBgDark', currentConfig.bgDark || '#0a0d12');
    setStudioFieldValue('uiBgDarkHex', currentConfig.bgDark || '#0a0d12');
    setStudioFieldValue('uiBgCard', currentConfig.bgCardHex || '#141820');
    setStudioFieldValue('uiBgCardHex', currentConfig.bgCardHex || '#141820');
    setStudioFieldValue('uiBorderColor', currentConfig.borderColorHex || '#1e2430');
    setStudioFieldValue('uiBorderHex', currentConfig.borderColorHex || '#1e2430');
    setStudioFieldValue('uiTextPrimary', currentConfig.textPrimary || '#ffffff');
    setStudioFieldValue('uiTextHex', currentConfig.textPrimary || '#ffffff');

    // 3. Populate Measurements
    setStudioFieldValue('uiSidebarWidth', currentConfig.sidebarWidth || 330);
    setStudioFieldValue('uiChannelsWidth', currentConfig.channelsWidth || 360);
    setStudioFieldValue('uiPlayerMaxWidth', currentConfig.playerMaxWidth || 1200);
    setStudioFieldValue('uiVodCols', currentConfig.vodCols || 5);
    setStudioFieldValue('uiVodGap', currentConfig.vodGap || 14);
    setStudioFieldValue('uiCardRadius', currentConfig.cardRadius || 12);

    // 4. Populate Texts & Ticker
    const tickerCb = document.getElementById('uiTickerEnabled');
    if (tickerCb) tickerCb.checked = !!currentConfig.tickerEnabled;
    setStudioFieldValue('uiTickerText', currentConfig.tickerText || '🔥 مرحباً بكم في سيرفرات الميزو - اشتراكات VIP بدون تقطيع مع دعم فني متواصل 24/7');
    setStudioFieldValue('uiTickerSpeed', currentConfig.tickerSpeed || 'normal');
    setStudioFieldValue('uiSiteTitle', currentConfig.siteTitle || 'سيرفرات الميزو - ALmEz0');
    setStudioFieldValue('uiWhatsappNumber', currentConfig.whatsappNumber || '+218945772649');

    // 5. Populate Visibility Toggles
    setStudioCheckbox('uiShowDeviceSwitcher', currentConfig.showDeviceSwitcher !== false);
    setStudioCheckbox('uiShowDownloads', currentConfig.showDownloads !== false);
    setStudioCheckbox('uiShowLiveSection', currentConfig.showLiveSection !== false);
    setStudioCheckbox('uiShowMoviesSection', currentConfig.showMoviesSection !== false);
    setStudioCheckbox('uiShowSeriesSection', currentConfig.showSeriesSection !== false);
    setStudioCheckbox('uiForceLowSpecMode', !!currentConfig.forceLowSpecMode);

    currentSelectedPreset = currentConfig.themePreset || 'emerald';
    highlightPresetCard(currentSelectedPreset);
    updateUiStudioPreview();
};

window.closeUiStudioModal = function () {
    const modal = document.getElementById('uiStudioModal');
    if (modal) modal.style.display = 'none';
};

window.switchUiStudioTab = function (tabId) {
    const tabs = ['themeTab', 'dimensionsTab', 'textsTab', 'visibilityTab'];
    const btnMap = {
        'themeTab': 'tabBtnTheme',
        'dimensionsTab': 'tabBtnDimensions',
        'textsTab': 'tabBtnTexts',
        'visibilityTab': 'tabBtnVisibility'
    };
...
function highlightPresetCard(presetKey) {
    const cards = document.querySelectorAll('.preset-theme-card');
    cards.forEach(card => card.classList.remove('active'));
    const names = ['emerald', 'ocean', 'gold', 'violet', 'crimson', 'titanium'];
    const idx = names.indexOf(presetKey);
    if (idx !== -1 && cards[idx]) {
        cards[idx].classList.add('active');
    }
}

window.selectPresetTheme = function (presetKey) {
    const preset = UI_STUDIO_PRESETS[presetKey];
    if (!preset) return;
    currentSelectedPreset = presetKey;
    highlightPresetCard(presetKey);

    setStudioFieldValue('uiPrimaryColor', preset.primaryColor);
    setStudioFieldValue('uiPrimaryHex', preset.primaryColor);
    setStudioFieldValue('uiAccentColor', preset.accentColor);
    setStudioFieldValue('uiAccentHex', preset.accentColor);
    setStudioFieldValue('uiBgDark', preset.bgDark);
    setStudioFieldValue('uiBgDarkHex', preset.bgDark);
    setStudioFieldValue('uiTextPrimary', preset.textPrimary);
    setStudioFieldValue('uiTextHex', preset.textPrimary);

    const badge = document.getElementById('previewThemeBadge');
    if (badge) badge.textContent = preset.name;

    updateUiStudioPreview();
};

window.syncColorPicker = function (pickerId, hexId) {
...
window.syncHexInput = function (hexId, pickerId) {
    const hex = document.getElementById(hexId);
    const picker = document.getElementById(pickerId);
...
window.updateUiStudioPreview = function () {
    // 1. Update dimension badges
    const sbW = getStudioFieldValue('uiSidebarWidth', '330');
    const chW = getStudioFieldValue('uiChannelsWidth', '360');
    const plW = getStudioFieldValue('uiPlayerMaxWidth', '1200');
    const vCols = getStudioFieldValue('uiVodCols', '5');
    const vGap = getStudioFieldValue('uiVodGap', '14');
    const cRad = getStudioFieldValue('uiCardRadius', '12');

    setElementText('valSidebarWidth', sbW + 'px');
    setElementText('valChannelsWidth', chW + 'px');
    setElementText('valPlayerMaxWidth', plW + 'px');
    setElementText('valVodCols', vCols + ' بطاقات');
    setElementText('valVodGap', vGap + 'px');
    setElementText('valCardRadius', cRad + 'px');

    // 2. Update preview colors
    const primaryColor = getStudioFieldValue('uiPrimaryColor', '#2e7d32');
    const accentColor = getStudioFieldValue('uiAccentColor', '#4caf50');
    const bgDark = getStudioFieldValue('uiBgDark', '#0a0d12');

    const previewBox = document.getElementById('studioPreviewBox');
    if (previewBox) {
        previewBox.style.setProperty('--mini-preview-primary', primaryColor);
        previewBox.style.setProperty('--mini-preview-accent', accentColor);
    }

    const activeCat = document.getElementById('previewMiniActiveCat');
    if (activeCat) activeCat.style.background = primaryColor;

    const playIcon = document.getElementById('previewMiniPlayIcon');
    if (playIcon) playIcon.style.color = accentColor;

    const miniCards = document.querySelectorAll('.preview-mini-card');
    miniCards.forEach(c => {
        c.style.borderRadius = cRad + 'px';
    });

    // 3. Update preview Ticker
    const tickerCb = document.getElementById('uiTickerEnabled');
    const tickerText = getStudioFieldValue('uiTickerText', '');
    const miniTicker = document.getElementById('previewMiniTicker');
    const miniTickerText = document.getElementById('previewMiniTickerText');

    if (miniTicker && miniTickerText) {
        if (tickerCb && tickerCb.checked && tickerText.trim()) {
            miniTicker.style.display = 'flex';
            miniTickerText.textContent = tickerText.substring(0, 65) + '...';
        } else {
            miniTicker.style.display = 'none';
        }
    }
};

window.saveUiStudioSettings = async function () {
    const saveBtn = document.getElementById('btnSaveStudio');
    if (saveBtn) {
        saveBtn.disabled = true;
        saveBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> جاري التعميم السحابي...';
    }
    ...
    try {
        ...
        const accentColor = getStudioFieldValue('uiAccentColor', '#4caf50');
        const bgDark = getStudioFieldValue('uiBgDark', '#0a0d12');
        const bgCardHex = getStudioFieldValue('uiBgCardHex', '#141820');
        const borderHex = getStudioFieldValue('uiBorderHex', '#1e2430');
        const textPrimary = getStudioFieldValue('uiTextPrimary', '#ffffff');

        const config = {
            themePreset: currentSelectedPreset || 'custom',
            primaryColor: primaryColor,
            accentColor: accentColor,
            glowColor: hexToRgba(accentColor, 0.35),
            bgDark: bgDark,
            bgCard: hexToRgba(bgCardHex, 0.85),
            bgCardHex: bgCardHex,
            borderColor: hexToRgba(borderHex, 0.15),
            borderColorHex: borderHex,
            textPrimary: textPrimary,
            textSecondary: '#94a3b8',

            sidebarWidth: parseInt(getStudioFieldValue('uiSidebarWidth', '330'), 10),
            channelsWidth: parseInt(getStudioFieldValue('uiChannelsWidth', '360'), 10),
            playerMaxWidth: parseInt(getStudioFieldValue('uiPlayerMaxWidth', '1200'), 10),
            vodCols: parseInt(getStudioFieldValue('uiVodCols', '5'), 10),
            vodGap: parseInt(getStudioFieldValue('uiVodGap', '14'), 10),
            cardRadius: parseInt(getStudioFieldValue('uiCardRadius', '12'), 10),

            siteTitle: getStudioFieldValue('uiSiteTitle', 'سيرفرات الميزو - ALmEz0'),
            tickerEnabled: !!(document.getElementById('uiTickerEnabled') && document.getElementById('uiTickerEnabled').checked),
            tickerText: getStudioFieldValue('uiTickerText', ''),
            tickerSpeed: getStudioFieldValue('uiTickerSpeed', 'normal'),
            tickerBg: `linear-gradient(90deg, ${primaryColor}, ${accentColor}, ${primaryColor})`,
            whatsappNumber: getStudioFieldValue('uiWhatsappNumber', '+218945772649'),

            showDeviceSwitcher: getStudioCheckbox('uiShowDeviceSwitcher'),
            showDownloads: getStudioCheckbox('uiShowDownloads'),
            showLiveSection: getStudioCheckbox('uiShowLiveSection'),
            showMoviesSection: getStudioCheckbox('uiShowMoviesSection'),
            showSeriesSection: getStudioCheckbox('uiShowSeriesSection'),
            forceLowSpecMode: getStudioCheckbox('uiForceLowSpecMode'),

            updatedAt: Date.now(),
            updatedBy: 'Admin'
        };

        // 1. Save to Firebase Firestore (siteConfig/ui_customization)
        if (typeof db !== 'undefined') {
            await db.collection('siteConfig').doc('ui_customization').set(config, { merge: true });
        }

        // 2. Cache locally
        localStorage.setItem('almezo_remote_ui_config', JSON.stringify(config));

        // 3. Apply live immediately
        if (window.AlMeZ0App && typeof window.AlMeZ0App.applyRemoteUiConfig === 'function') {
            window.AlMeZ0App.applyRemoteUiConfig(config);
        }

        closeUiStudioModal();

        Swal.fire({
            title: 'تم تعميم الواجهة بنجاح! 🚀',
            text: 'تم تطبيق الألوان والقياسات والنصوص فورياً على كافة أجهزة الأندرويد، شاشات التلفزيون، الكمبيوتر، والموقع في ثانية واحدة دون الحاجة لتحديث التطبيق!',
            icon: 'success',
            background: '#0d111a',
            color: '#fff',
        ...
        console.error('Failed to save UI Studio settings:', err);
        Swal.fire({
            title: 'خطأ أثناء الحفظ',
            text: err.message || 'تعذر حفظ الإعدادات السحابية. يرجى التحقق من الاتصال بالإنترنت.',
            icon: 'error',
            background: '#0d111a',
            color: '#fff'
        });
        ...
        if (saveBtn) {
            saveBtn.disabled = false;
            saveBtn.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> حفظ وتطبيق فوراً على جميع الأجهزة';
        }
    }
};

window.resetUiStudioDefaults = async function () {
    const confirm = await Swal.fire({
        title: 'استعادة الإعدادات الأصلية؟',
        text: 'هل أنت متأكد من رغبتك في إعادة ضبط الألوان، القياسات، والنصوص إلى القيم الافتراضية الأصلية للمشغل والموقع؟',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'نعم، استعد الأصلي',
        cancelButtonText: 'إلغاء',
        background: '#0d111a',
        color: '#fff',
    ...
    if (confirm.isConfirmed) {
        ...
        setStudioFieldValue('uiSidebarWidth', 330);
        setStudioFieldValue('uiChannelsWidth', 360);
        setStudioFieldValue('uiPlayerMaxWidth', 1200);
        setStudioFieldValue('uiVodCols', 5);
        setStudioFieldValue('uiVodGap', 14);
        setStudioFieldValue('uiCardRadius', 12);
        setStudioCheckbox('uiTickerEnabled', false);
        setStudioFieldValue('uiTickerText', '🔥 مرحباً بكم في سيرفرات الميزو - اشتراكات VIP بدون تقطيع مع دعم فني متواصل 24/7');
        setStudioFieldValue('uiTickerSpeed', 'normal');
        setStudioFieldValue('uiSiteTitle', 'سيرفرات الميزو - ALmEz0');
        setStudioFieldValue('uiWhatsappNumber', '+218945772649');
        setStudioCheckbox('uiShowDeviceSwitcher', true);
        setStudioCheckbox('uiShowDownloads', true);
        setStudioCheckbox('uiShowLiveSection', true);
        setStudioCheckbox('uiShowMoviesSection', true);
        setStudioCheckbox('uiShowSeriesSection', true);
        setStudioCheckbox('uiForceLowSpecMode', false);

        updateUiStudioPreview();
        saveUiStudioSettings();
    }
};

// Studio Helper Functions
function getStudioFieldValue(id, fallback) {
    const el = document.getElementById(id);
    return el ? el.value : fallback;
}

function setStudioFieldValue(id, val) {
    const el = document.getElementById(id);
    if (el) el.value = val;
}

function getStudioCheckbox(id) {
    const el = document.getElementById(id);
    return el ? el.checked : true;
}

function setStudioCheckbox(id, checked) {
    const el = document.getElementById(id);
...
// Auto open Studio if requested via URL (?openStudio=true)
document.addEventListener('DOMContentLoaded', () => {
    try {
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.get('openStudio') === 'true') {
            setTimeout(() => {
                if (typeof openUiStudioModal === 'function') openUiStudioModal();
            }, 600);
        }
    } catch (e) { }
});
