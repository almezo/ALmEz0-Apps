...
def foreground(logo, size):
    ...
    المقاس 88dp من 108dp (ملء كامل): المنطقة الظاهرة بعد القص هي 72dp، ومحتوى اللوقو داخل
    إطاره يشغل 83% منه، فبهذا المقاس يملأ المحتوى النافذة تماماً ويبقى الإطار الأخضر خارجها.

    النتيجة المقصودة: الجهاز الذي يقصّ (سامسونج) يعرض اللوقو مالئاً الأيقونة بلا فراغ حوله،
    والجهاز الذي لا يقصّ (والأيقونة المربعة القديمة) يعرضه بإطاره الأخضر كاملاً.
    جرّبنا 55dp فظهر صغيراً داخل مربع أسود، و72dp فقُصّ إطاره وبدا ناقصاً.
    """
    n = size * SS
    img = Image.new('RGBA', (n, n), (0, 0, 0, 0))
    # ملء كامل: المحتوى يملأ المنطقة الظاهرة والإطار الأخضر يقع خارج حدود القص
    box = int(n * 88.0 / 108.0)
